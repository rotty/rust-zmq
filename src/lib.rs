...
impl SocketEvent {
    fn to_raw(self) -> u16 {
        use SocketEvent::*;
        let raw = match self {
            CONNECTED => zmq_sys::ZMQ_EVENT_CONNECTED,
            CONNECT_DELAYED => zmq_sys::ZMQ_EVENT_CONNECT_DELAYED,
            CONNECT_RETRIED => zmq_sys::ZMQ_EVENT_CONNECT_RETRIED,
            LISTENING => zmq_sys::ZMQ_EVENT_LISTENING,
            BIND_FAILED => zmq_sys::ZMQ_EVENT_BIND_FAILED,
            ACCEPTED => zmq_sys::ZMQ_EVENT_ACCEPTED,
            ACCEPT_FAILED => zmq_sys::ZMQ_EVENT_ACCEPT_FAILED,
            CLOSED => zmq_sys::ZMQ_EVENT_CLOSED,
            CLOSE_FAILED => zmq_sys::ZMQ_EVENT_CLOSE_FAILED,
            DISCONNECTED => zmq_sys::ZMQ_EVENT_DISCONNECTED,
            MONITOR_STOPPED => zmq_sys::ZMQ_EVENT_MONITOR_STOPPED,
            HANDSHAKE_FAILED_NO_DETAIL => zmq_sys::ZMQ_EVENT_HANDSHAKE_FAILED_NO_DETAIL,
            HANDSHAKE_SUCCEEDED => zmq_sys::ZMQ_EVENT_HANDSHAKE_SUCCEEDED,
            HANDSHAKE_FAILED_PROTOCOL => zmq_sys::ZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL,
            HANDSHAKE_FAILED_AUTH => zmq_sys::ZMQ_EVENT_HANDSHAKE_FAILED_AUTH,
            ALL => zmq_sys::ZMQ_EVENT_ALL,
        };
        raw as u16
    }
...
impl Socket {
    ...
    pub fn monitor(&self, monitor_endpoint: &str, events: SocketEvent) -> Result<()> {
        ...
        zmq_try!(unsafe {
            zmq_sys::zmq_socket_monitor(self.sock, c_str.as_ptr(), events.to_raw() as c_int)
        });
        Ok(())
    }
    ...
    pub fn get_gssapi_principal(&self) -> Result<result::Result<String, Vec<u8>>> {
        ...
        sockopt::get_string(
            self.sock,
            zmq_sys::ZMQ_GSSAPI_PRINCIPAL as c_int,
            260,
            true,
        )
    }
